...
#' Retrieve draft assets using an existing connection
#' @noRd
get_draft_assets_from_connection <- function(
    con,
    team_value,
    year_from = NULL,
    year_to = NULL,
  ...
  if (nrow(result)) {
    ...
    original_id <- as.integer(
      result$perspective_original_team_id
    )
    current_id <- as.integer(
      result$perspective_current_team_id
    )
    result$control_type <- vapply(
      seq_len(nrow(result)),
      function(i) {
