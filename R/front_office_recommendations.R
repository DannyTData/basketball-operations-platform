...
# ============================================================
# PHASE 3.1 OVERRIDE — CURRENT ROSTER RECOMMENDATION CONTEXT
# ============================================================
...
p3s11_get_integrated_team_roster <- function(
  ...
  if (
    ...
    if (
      exists(
        "attach_tbi_current_contract_context",
        mode = "function",
        inherits = TRUE
      )
    ) {
  ...
  stop(
    "Current roster evaluation context is not available."
  )
}
...
run_phase3_recommendation_package <- function(
  ...
  # Existing calibrated builder now receives its roster through
  # the Phase 3.1 p3s11_get_integrated_team_roster() override.
  player_recommendations <- build_phase3_player_recommendations(
    team_name = team_name,
    season = season,
    con = con
  )
  
  priorities <- build_phase3_team_priorities(
    team_name = team_name,
    season = season,
    con = con
  )
  
  sanity_flags <- phase3_step11_sanity_check(
    player_recommendations
  )
  
  recommendation_counts <- if (
    nrow(player_recommendations)
  ) {
    as.data.frame(
      sort(
        table(
          player_recommendations$recommendation
        ),
        decreasing = TRUE
      )
    )
  ...
  if (nrow(recommendation_counts)) {
    names(recommendation_counts) <- c(
      "recommendation",
      "players"
    )
  }
  
  current_roster <- p3s11_get_integrated_team_roster(
    team_name = team_name,
    season = season,
    con = con,
    current_roster_season = current_roster_season
  ...
  list(
    ...
    sanity_flag_count = nrow(sanity_flags),
    ...
    recommendation_scope =
      "CURRENT-ROSTER CONTEXT + PRIOR-SEASON PERFORMANCE EVIDENCE"
  )
}